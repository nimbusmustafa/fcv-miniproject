...
def FindBoxCoordinates(Frame):
    ...
    # If no aruco marker found or number of aruco markers found is not 4, return None
...
if __name__ == "__main__":
    # Reading aruco video.
    ArucoVid_Cap = cv2.VideoCapture("videoplayback.mp4")

    # Reading video for projection
    ProjVid_Cap = cv2.VideoCapture("got.mp4")

    # Creating video writer object
    OutVid = cv2.VideoWriter('Videos/FinalVideo_py.mp4', cv2.VideoWriter_fourcc(*'mp4v'), ArucoVid_Cap.get(cv2.CAP_PROP_FPS), (int(ArucoVid_Cap.get(3)), int(ArucoVid_Cap.get(4)))) 
